...
def hd_2D_stack(result, reference, pixelspacing=None, connectivity=1):
    ...
    for i in range(zdim):
        if (np.sum(result[i, :, :]) > 0 and np.sum(reference[i, :, :]) > 0):
            hausdorff_distance = hd(result[i], reference[i], voxelspacing=pixelspacing, connectivity=connectivity)
            c = c + 1
...
def VolumeSimIndex(result, reference):
    """
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4533825/#Equ21
      volume similarity=  1- abs(\frac{(v1-v2)})(abs({v1+v2})
    Parameters
    ----------
    result : array_like
        Input data containing objects. Can be any type but will be converted
    ...
    return 1 - np.abs(vol1 - vol2) / np.abs(float(vol2 + vol1))
