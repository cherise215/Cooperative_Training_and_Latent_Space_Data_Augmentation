# Generate a number of adversarial images for transferable attack.
# input model unet-16, pretrained on healthy subjects (n=16)
# providing the test cases to generate adversarial counterparts,
...
if __name__ == '__main__':
    ...
    id_list = [7, 8, 9, 10, 27, 28, 29, 30, 47, 48, 49, 50, 67, 68, 69, 70, 87, 88, 89, 90]
    test_image_paths = [[str(pid).zfill(3), join(sub_dir, '{}_img.nrrd').format(str(pid).zfill(3))] for pid in id_list]
    test_label_paths = [[str(pid).zfill(3), join(sub_dir, '{}_seg.nrrd').format(str(pid).zfill(3))] for pid in id_list]
    ...
    for attack_name, attacker in attackers.items():
        for j in range(n_augmented):
            for i in range(len(test_image_paths)):
                sitkImage = load_img_label_from_path(img_path=test_image_paths[i][1])
                ...
                origin_image3D_tensor = torch.tensor(image[:, np.newaxis, :, :], requires_grad=False).float()

                attacked_image = attacker(origin_image3D_tensor.permute(1, 0, 2, 3))  # NCHW->CNHW
                attacked_image = attacked_image.permute(1, 0, 2, 3)  # CNHW->NCHW

                attacked_image = rescale_intensity(attacked_image, new_min=0, new_max=1)
                ...
                patient_dir = join(save_dir, *[attack_name, str(pid) + '_' + str(j)])
                ...
                save_medical_image(sitk.GetImageFromArray(adv_image), output_path=file_path, refImage=sitkImage)

                # label path
                label_path = test_label_paths[i][1]
                target_label_path = join(patient_dir, '{}_label.nrrd'.format(frame))
